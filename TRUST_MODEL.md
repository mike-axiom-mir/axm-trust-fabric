...
The reference test reproduces every fixed value exactly and verifies the signature directly with Node's Ed25519 primitive. Any mismatch is a failure rather than something to normalize silently.

A second implementation path now exists at `independent/vector-verifier-v1.js`. It does not import `src/trust-core.js`; its test enforces that dependency boundary and independently recomputes the fixed vector's canonical bytes, key id, digest, signature result, trusted-root decision, time decision, and exact scope decision. It also refuses mutated signed bytes, an untrusted root, expiry, wrong target, and wrong action.

This establishes **same-repository source-level implementation separation for one fixed vector**. It does not establish independent third-party interoperability, cross-language conformance, or production protocol certification.
...
- agreement between two same-repository implementations does not equal third-party interoperability;
...
- the separate verifier is evidence-only and does not replace the reference runtime.
...
- interoperability grew from one fixed vector to one bounded separate verifier before any protocol service or compatibility claim;
